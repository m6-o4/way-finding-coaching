...
- **Visual pattern**: `bg-background`/`bg-muted` section (`py-16 lg:py-30`) wrapping a `Container`; headline `font-heading text-3xl md:text-4xl font-semibold text-foreground` with "View All Articles" as a `Button` `secondary` variant rendered as a `Link` (`nativeButton={false}`, hidden below `md`); cards in a `grid gap-8 md:grid-cols-3`, each a `Link`-wrapped shadcn `Card` conformed to the theme — `group h-full cursor-pointer gap-0 overflow-hidden rounded-lg border border-card-border p-0 ring-0 transition-colors` (overrides the stock `rounded-xl`/`ring-foreground/10`/`gap-(--card-spacing)`/`py-(--card-spacing)`, re-adds plain `group` since `Card` ships the named `group/card`; no shadow), a `bg-muted aspect-16/10` image area with a `Badge variant="secondary"` category label, and a `CardHeader` `block flex-1 p-6` body with `text-muted-foreground text-xs` date, a `CardTitle` `text-foreground text-xl font-semibold` (`group-hover:text-primary` + `transition-colors`), and a `CardDescription` `text-sm` line-clamped description. No shadows.
...
- **Props**: the `MeetMichelle` block (`backgroundVariant` `background` | `muted`, `headline` (required), `headlineDescription`, `bio` richText (required), `photo` upload → media (required)).
- **Visual pattern**: `bg-background`/`bg-muted` section (`py-16 lg:py-30`) wrapping a `Container`; header `mb-12 text-center` renders `headline` (`h2`, `text-foreground mb-4 text-3xl font-semibold md:text-4xl`) + optional `headlineDescription` (`text-muted-foreground`); `grid items-start gap-12 lg:grid-cols-3` with the photo in `lg:col-span-1` (a `bg-muted relative aspect-4/5 w-full overflow-hidden rounded-lg border border-card-border` wrapper holding `next/image` `fill` `object-cover`, rendered only when `photo` is a populated object) and the bio in `lg:col-span-2` via `RichText` (`enableGutter={false}`), rendered unconditionally. No shadows, no hardcoded hex.
- **Used in**: `pages` documents via the `meetMichelle` block, registered in `src/payload/blocks/render-blocks.tsx`.

### `SocialProofBlock`
- **Location**: `src/payload/blocks/social-proof/component.tsx`
- **Purpose**: a testimonials carousel — shows one testimonial at a time, navigated with prev/next buttons.
- **Props**: the `SocialProof` block (`backgroundVariant` `background` | `muted`, `headline` (required), `headlineDescription`, `testimonials` array of `name` / `photo` (upload → media) / `jobTitle` / `testimony`).
